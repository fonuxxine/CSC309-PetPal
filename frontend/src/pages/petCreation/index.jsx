...
function PetCreation() {
    ...
    return <>
        <div className="container-fluid return-to-bar d-flex justify-content-start p-3 return-to-app">
            <button className="btn btn-default btn-outline-dark detail-btn"
                    onClick={() => navigate("/shelter/manage/")}
            >Return to Home</button>
        </div>
        {error !== "" ? (
            <h1 className="text-center p-0">{error}</h1>
          ) : (
            <></>
          )}
        <div className="application-form">
            {photo !== "" ? <div className="profile-picture">
                        ...
                        <label htmlFor="gender">Gender</label>
                          ...
                          >
                              {Object.entries(genderValues).map(([i, gender]) =>
                                      <option value={gender} key={i}>{genderValues[gender]}</option>
                              )}
                          </select>
                    </div>
                    <div className="col-md-6 p-2">
